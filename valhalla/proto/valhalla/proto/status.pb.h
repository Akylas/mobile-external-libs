...
inline void Status::set_bbox(ArgT0&& arg0, ArgT... args) {
  if (!_internal_has_bbox()) {
    clear_has_bbox();
    set_has_bbox();
    has_bbox_.bbox_.InitDefault();
  }
  has_bbox_.bbox_.Set( static_cast<ArgT0 &&>(arg0), args..., GetArenaForAllocation());
...
inline void Status::_internal_set_bbox(const std::string& value) {
  ...
  has_bbox_.bbox_.Set(value, GetArenaForAllocation());
}
...
inline void Status::set_allocated_bbox(std::string* bbox) {
  ...
  if (bbox != nullptr) {
    set_has_bbox();
    has_bbox_.bbox_.InitAllocated(bbox, GetArenaForAllocation());
  }
