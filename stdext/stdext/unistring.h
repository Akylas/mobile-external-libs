#ifndef _UNISTRING_H_INCLUDED_
#define _UNISTRING_H_INCLUDED_
...
namespace unistring {
    namespace detail {
        ...
        inline std::uint32_t to_lower(std::uint32_t c) {
            ...
            return (it->first != c ? c : it->second);
        }

        inline std::uint32_t to_upper(std::uint32_t c) {
            static const std::initializer_list<std::pair<std::uint16_t, std::uint16_t>> table = {
    ...
    inline unistring to_unistring(const std::wstring& wstr) {
        if (sizeof(wchar_t) == 2) {
            std::string utf8str;
            utf8::utf16to8(wstr.begin(), wstr.end(), std::back_inserter(utf8str));
            return to_unistring(utf8str);
        }
        unistring unistr;
        unistr.insert(unistr.end(), wstr.begin(), wstr.end());
        return unistr;
    }

    inline std::string to_utf8string(const unistring& unistr) {
        std::string utf8str;
        utf8::utf32to8(unistr.begin(), unistr.end(), std::back_inserter(utf8str));
        return utf8str;
    }

    inline std::wstring to_wstring(const unistring& unistr) {
        std::wstring wstr;
        if (sizeof(wchar_t) == 2) {
            std::string utf8str = to_utf8string(unistr);
    ...
    inline unistring to_upper(const unistring& str) {
        unistring result = str;
        std::transform(result.begin(), result.end(), result.begin(), detail::to_upper);
        return result;
    }

    inline unistring to_lower(const unistring& str) {
        unistring result = str;
        std::transform(result.begin(), result.end(), result.begin(), detail::to_lower);
        return result;
    }

    inline unistring capitalize(const unistring& str) {
        unistring result = str;
        if (!result.empty()) {
            result[0] = detail::to_upper(result[0]);
        }
        return result;
    }
}
